...
import { useDocuments, useReaderChat, useReaderPlayback } from "@/state/app";
...
export default function ReaderScreen() {
  ...
  const findById = useDocuments((s) => s.findById);
  const doc = findById(params.id);

  const tweaks = useTweaks();
  const playback = useReaderPlayback();
  const chat = useReaderChat((s) => s.messages);
  const ask = useReaderChat((s) => s.ask);
  ...
  useEffect(() => {
    playback.setTotalWords(words.length);
    return () => {
      Speech.stop();
  ...
  const togglePlay = useCallback(async () => {
    ...
    Speech.speak(remaining, {
      pitch,
      rate,
      onDone: () => playback.pause(),
      onStopped: () => playback.pause(),
      onError: () => playback.pause(),
    });

    if (simTimerRef.current) clearInterval(simTimerRef.current);
    simTimerRef.current = setInterval(() => {
      useReaderPlayback.getState().advance();
      if (!useReaderPlayback.getState().playing && simTimerRef.current) {
