...
import { selectDocById } from "@/store/slices/documents";
import { askReader } from "@/store/slices/readerChat";
import {
  advance as advanceWord,
  pauseReader,
  playReader,
  setTotalWords,
} from "@/store/slices/readerPlayback";
import { setVoice, setWpm } from "@/store/slices/tweaks";
...
export default function ReaderScreen() {
  ...
  const doc = selectDocById(docs, params.id);

  const tweaks = useAppSelector((s) => s.tweaks);
  const playback = useAppSelector((s) => s.readerPlayback);
  const chat = useAppSelector((s) => s.readerChat.messages);
  const ask = (args: { paragraphId: string; question: string }) =>
    dispatch(askReader(args));
  ...
  useEffect(() => {
    dispatch(setTotalWords(words.length));
    return () => {
      Speech.stop();
  ...
  const togglePlay = useCallback(async () => {
    ...
    const pitch = tweaks.voice === "deep" ? 0.85 : tweaks.voice === "natural" ? 1.0 : 1.1;
    const rate = Math.min(1, Math.max(0.3, tweaks.wpm / 400));
    ...
    simTimerRef.current = setInterval(() => {
      dispatch(advanceWord());
      if (!store.getState().readerPlayback.playing && simTimerRef.current) {
        clearInterval(simTimerRef.current);
      }
    }, 60000 / tweaks.wpm);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [playback.playing, playback.wordIndex, tweaks.wpm, tweaks.voice, words]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: palette.surface }}>
      ...
      <View style={{ flex: 1, flexDirection: "row" }}>
        {tweaks.readerLayout !== "paginated" ? (
          <Minimap sections={doc.sections ?? []} progress={progress} />
        ) : null}
        <ReaderBody
          doc={doc}
          progress={progress}
      ...
      <PlaybackBar
        ...
        voice={tweaks.voice}
        onPlay={togglePlay}
        onWpm={(v) => dispatch(setWpm(v))}
        onVoice={(v) => dispatch(setVoice(v))}
        onAsk={() => {
