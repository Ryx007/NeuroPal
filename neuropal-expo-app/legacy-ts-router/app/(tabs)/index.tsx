...
export default function HomeScreen() {
  ...
  const remaining = useAppSelector(selectRemainingTasks);
  const anchors = useAppSelector(selectAnchors);
  const docs = useAppSelector(selectDocuments);
  const nervous = useAppSelector(selectNervousState);
  const nextAnchor = useAppSelector(selectNextAnchor);
  const resume = useAppSelector(selectResumeDocument);
      ...
      <View style={{ marginTop: 30 }}>
        ...
        {tasks.map((t) => (
          <MvdRow
            key={t.id}
            task={t}
            onToggle={() => dispatch(toggleTask(t.id))}
          />
        ))}
      </View>
