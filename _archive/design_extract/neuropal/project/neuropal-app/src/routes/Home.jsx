...
import { setUserState } from '../../store/slices/uiSlice';
import { LIBRARY_ITEMS } from '../../data/papers';

// ─── State check-in ──────────────────────────────────────────────────
function StateCheckIn({ userState, dispatch }) {
  const rows = [
      ...
      <Box sx={{ display: 'grid', gap: 1.25 }}>
        {rows.map(({ id, Icon, label, sub, color }) => {
          ...
          return (
            <Box
              key={id}
              onClick={() => dispatch(setUserState(id))}
              sx={{
  ...
  return (
    <Box sx={{ px: 5, pb: 7.5, overflow: 'auto', height: '100%' }}>
      <Box sx={{ maxWidth: 1200, mx: 'auto', display: 'grid', gap: 2.5 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 2.5 }}>
          <StateCheckIn userState={userState} dispatch={dispatch} />
          <Box sx={{ display: 'grid', gap: 2.5, gridAutoRows: 'min-content' }}>
            <AnchorCard />
            <MVDCard />
          </Box>
        </Box>
