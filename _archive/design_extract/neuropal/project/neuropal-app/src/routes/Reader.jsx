// ─── Reader page — the heart of NeuroPal ─────────────────────────────
// Karaoke TTS, inline margin Q&A, minimap, citation graph, focus modes
...
import { setWpm, setVoice } from '../../store/slices/uiSlice';
import {
  ...
  return (
    <Box className="glass" sx={{
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
        <IconButton size="small" onClick={() => dispatch(seekBack(totalWords))}>
...
// ─── Reader page assembled ───────────────────────────────────────────
function ReaderBase({
  activePaperId, playing, globalWordIndex, focusMode, showNotes, showGraph,
  wpm, voice, readerFont, readerLayout, fontSize, lineSpacing,
}) {
  ...
  return (
    <Box sx={{ position: 'relative', height: '100%', bgcolor: '#131313', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Header strip */}
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.75, px: 5, pt: 2.25, pb: 1.25 }}>
      ...
      <Box sx={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        {layout !== 'paginated' && (
          <Box sx={{ width: 220, flexShrink: 0, pl: 3.5, pt: 2.5 }}>
            <Minimap paper={paper} currentSection={currentSection} progress={progress} onJump={jumpSection} />
          </Box>
        )}

        <Box sx={{ flex: 1, overflow: 'auto', px: 5, pt: 3.5, pb: 25, position: 'relative' }}>
...
const Reader = connect(mapStateToProps)(ReaderBase);
